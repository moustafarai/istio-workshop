...
export class ServiceC extends Component {
  static displayName = ServiceC.name;
  ...
  componentDidMount() {
    this.populateServiceC();
  }

  static renderServiceC(service) {
    return (
  ...
  render() {
    let contents = this.state.loading
      ? <p><em>Loading...</em></p>
      : ServiceC.renderServiceC(this.state.service);
  ...
  async populateServiceC() {
    const response = await fetch('/ServiceC');
