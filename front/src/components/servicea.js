...
export class ServiceA extends Component {
  static displayName = ServiceA.name;
  ...
  componentDidMount() {
    this.populateServiceA();
  }

  static renderServiceA(service) {
    return (
  ...
  render() {
    let contents = this.state.loading
      ? <p><em>Loading...</em></p>
      : ServiceA.renderServiceA(this.state.service);
  ...
  async populateServiceA() {
    const response = await fetch('/ServiceA');
